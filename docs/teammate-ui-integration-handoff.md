...
2. 角色播放走路动画并移动到落点附近。
3. 角色播放指向动画。
4. 显示二选一确认：`是的` / `嘿嘿嘿，就是它`。
5. 角色播放踢击动画；交接流程所称的“第 5 帧”触发爆炸，同时在 target 模式请求删除。现有代码的准确触发条件是零基索引 `frameIndex === 5`，也就是精灵表中的第 6 格。
6. 角色播放登场动画。
7. 角色播放飞离动画并移出屏幕。
8. 等待爆炸和删除结果；失败时显示提示，然后关闭 overlay。

产品口径中的第二个按钮是“嘿嘿嘿，就是它”。当前 `src/renderer/index.html` 中仍写作“嘿嘿嘿，就是这个”，UI 接手时应统一文案，但不能改变两枚按钮都只用于继续同一条确认流程的行为。
...
- **target 模式**：由启动参数 `--target "<path>"` 进入。`targetPath` 在应用启动时由主进程解析并保存在本次会话中；踢击第 5 帧才会请求把该目标移入回收站。
...
| `src/renderer/main.ts` | 完整体验状态流、点击落点、音频、动画编排、踢击第 5 帧删除、错误提示和关闭。 |
...
| 踢击 | `assets/踹文件动效_spritesheet_transparent.png` | 5 × 3 | 15 帧；索引 5 触发爆炸与删除请求 | 1122 × 1402 px |
...
- 踢击触发帧（现有实现为零基 `frameIndex === 5`）触发爆炸与 target 模式删除请求的时序语义。
...
- [ ] “是的”与“嘿嘿嘿，就是它”都能继续同一确认流程，按钮可点击、可聚焦且光标为 pointer。
- [ ] 走路、指向、踢击、登场按 5 × 3 切分；飞离按 4 × 4、16 帧切分；爆炸按 5 × 3 切分。
- [ ] 指向只播放 `[11, 12, 13, 14]`；踢击索引 5 只触发一次爆炸和一次删除请求。
